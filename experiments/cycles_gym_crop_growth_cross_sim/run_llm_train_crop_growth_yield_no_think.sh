...
RUN_NAME="cycles_gym_crop_growth_cross_sim_llm_no_think_train"
DATASET_CONFIG="$SCRIPT_DIR/config/crop_growth_yield_llm_no_think.yaml"
DATASET_ID="cycles_gym_crop_growth_cross_sim_llm_no_think"
...
if [[ 1 -eq 1 || ! -f "$TRAIN_FILE" || ! -f "$VAL_FILE" || ! -f "$TEST_FILE" || "$DATASET_CONFIG" -nt "$TRAIN_FILE" || "$DATASET_CONFIG" -nt "$VAL_FILE" || "$DATASET_CONFIG" -nt "$TEST_FILE" || "$PROMPT_FILE" -nt "$TRAIN_FILE" || "$DATASET_BUILDER" -nt "$TRAIN_FILE" ]]; then
...
exec bash entrypoints/train/train.sh \
    ...
    "trainer.project_name=agrimanager_cycles_gym_crop_growth_cross_sim" \
