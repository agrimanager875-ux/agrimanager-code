...
RUN_NAME="cycles_gym_crop_planning_smoke_nn_eval"
TRAIN_RUN_NAME="cycles_gym_crop_planning_smoke_nn_train"
...
if ! AGENT_PATH="$(latest_agent)"; then
    ...
    echo "Run smoke_tests/cycles_gym/run_nn_train_crop_planning.sh first." >&2
    exit 1
fi
