...
RUN_NAME="wofost_gym_smoke_nn_eval"
TRAIN_RUN_NAME="wofost_gym_smoke_nn_train"
...
if ! AGENT_PATH="$(latest_agent)"; then
    ...
    echo "Run smoke_tests/wofost_gym/run_nn_train.sh first." >&2
    exit 1
fi
