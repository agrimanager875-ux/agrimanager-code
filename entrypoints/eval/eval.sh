...
DEFAULT_CONFIG_DIR="$PROJECT_DIR/entrypoints/eval/config"
...
if [[ -n "$LOG_FILE" ]]; then
    ...
    python -u -m agrimanager.rollout.inference.inference_rollout \
        "${DEFAULT_ARGS[@]}" "${FORWARD_ARGS[@]}" \
        2>&1 | tee "$LOG_FILE"
else
    exec python -u -m agrimanager.rollout.inference.inference_rollout \
        "${DEFAULT_ARGS[@]}" "${FORWARD_ARGS[@]}"
fi
